...
#music
tf.flags.DEFINE_string("valid_data", "/content/drive/My Drive/datatemp/music/valid.csv", " Data for validation")
tf.flags.DEFINE_string("train_data", "/content/drive/My Drive/datatemp/music/train.csv", "Data for training")
tf.flags.DEFINE_string("user_review", "/content/drive/My Drive/datatemp/music/user_review", "User's reviews")
tf.flags.DEFINE_string("item_review", "/content/drive/My Drive/datatemp/music/item_review", "Item's reviews")
tf.flags.DEFINE_string("user_review_id", "/content/drive/My Drive/datatemp/music/user_rid", "user_review_id")
tf.flags.DEFINE_string("item_review_id", "/content/drive/My Drive/datatemp/music/item_rid", "item_review_id")
tf.flags.DEFINE_string("rating_matrix","/content/drive/My Drive/datatemp/music/dmfcur/rating_matrix", " rating matrix ")
print("load music")
...
def build_input_data(u_text, i_text, vocabulary_u, vocabulary_i):
    ...
    for i in u_text.keys():
        u_reviews = u_text[i]
        u = np.array([[vocabulary_u[word] for word in words] for words in u_reviews]) #dif
        u_text2[i] = u
    ...
    for j in i_text.keys():
        i_reviews = i_text[j]
        i = np.array([[vocabulary_i[word] for word in words] for words in i_reviews]) #dif
        i_text2[j] = i
    return u_text2, i_text2


def load_data(train_data, valid_data, user_review, item_review, user_rid, item_rid):  #dif 去掉停用词这个不错
    """
...
def load_data_and_labels(train_data, valid_data, user_review, item_review, user_rid, item_rid):
    ...
    x2 = np.sort(review_len_u)     #对每个评论长度排序
...
if __name__ == '__main__':
    ...
    dmf_userid_train = userid_train
    dmf_itemid_train = itemid_train
    dmf_y_train = y_train
    dmf_userid_valid = uid_valid
    dmf_itemid_valid = iid_valid
    dmf_y_valid = y_valid
    ...
    batches_train = list(zip(dmf_userid_train, dmf_itemid_train, dmf_y_train, userid_train, itemid_train, reid_user_train, reid_item_train, y_train))
    batches_test = list(zip(dmf_userid_valid, dmf_itemid_valid, dmf_y_valid, userid_valid, itemid_valid, reid_user_valid, reid_item_valid, y_valid))

    print ('Write begin. Good luck !')
    ...
    f_rating_matrix = open(FLAGS.rating_matrix, "rb")
    rating_matrix = pickle.load(f_rating_matrix)
    user_total_num = rating_matrix.shape[0]
    item_total_num = rating_matrix.shape[1]


    para = {}
    para['user_num'] = user_num
    para['item_num'] = item_num
    para['user_total_num'] = user_total_num
    para['item_total_num'] = item_total_num
    ...
    print("Para dumpping done!!Good luck !")
